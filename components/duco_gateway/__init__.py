...
CONFIG_SCHEMA = (
    cv.Schema({
        ...
        cv.Optional(CONF_EXT_SENSOR_NODE): cv.uint8_t,
        cv.Optional(CONF_RH_SENSOR_NODE): cv.uint8_t,

        # Optional sensors/text sensors
...
async def to_code(config):
    ...
    if CONF_EXT_SENSOR_NODE in config:
        cg.add(var.set_ext_sensor_node(config[CONF_EXT_SENSOR_NODE]))

    if CONF_RH_SENSOR_NODE in config:
